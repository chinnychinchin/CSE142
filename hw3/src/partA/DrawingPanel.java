 ...
 * =====================================================================
 *
 * COMPATIBILITY NOTE: This version of partA.DrawingPanel requires Java 8 or higher.
 ...
 * =====================================================================
 * 
 * The partA.DrawingPanel class provides a simple interface for drawing persistent
 * images using a Graphics object.  An internal BufferedImage object is used
 * to keep track of what has been drawn.  A client of the class simply
 * constructs a partA.DrawingPanel of a particular size and then draws on it with
...
/**
 *
 * partA.DrawingPanel is a simplified Java drawing window class to accompany
 ...
 * 
 * <p>
 * You can always download the latest {@code partA.DrawingPanel} from
 ...
 * 
 * <p>
 * The {@code partA.DrawingPanel} class provides a simple interface for drawing persistent
 * images using a {@code Graphics} object.  An internal {@code BufferedImage} object is used
 * to keep track of what has been drawn.  A client of the class simply
 * constructs a {@code partA.DrawingPanel} of a particular size and then draws on it with
 ...
 * {@code partA.DrawingPanel} is not intended to be a full rich graphical library for things
 ...
 * 
 * <p>
 * Here is a canonical example of creating a {@code partA.DrawingPanel} of a given size and
 * using it to draw a few shapes.
 * </p>
 * 
 * <pre>
 * // basic usage example
 * partA.DrawingPanel panel = new partA.DrawingPanel(600, 400);
 ...
 * 
 * <p>
 * This version of {@code partA.DrawingPanel} allows you to loop over the pixels of an image.
 ...
 * public static void flipHorizontal(partA.DrawingPanel panel) {
 ...
 * events that occur in a {@code partA.DrawingPanel} using a lambda function.  For example:
 * 
 * <pre>
 ...
 * It includes a {@code showCounts} method for the {@code partA.DrawingPanel} itself
...
public final class DrawingPanel implements ImageObserver {
    ...
    private static final String ABOUT_MESSAGE       = "partA.DrawingPanel\n"
    ...
    private static final String ABOUT_MESSAGE_TITLE = "About partA.DrawingPanel";
    ...
    /**
     * The default width of a partA.DrawingPanel in pixels, if none is supplied at construction (500 pixels).
     */
    public static final int DEFAULT_WIDTH           = 500;
    
    /**
     * The default height of a partA.DrawingPanel in pixels, if none is supplied at construction (400 pixels).
     */
    ...
    // last partA.DrawingPanel is closed
    static {
        try {
    ...
    /*
     * Called when partA.DrawingPanel class loads up.
    ...
    private static void checkAnimationSettings() {
        try {
            ...
            if (settingsFile.exists()) {
                ...
                input.close();
                System.out.println("***");
                System.out.println("*** partA.DrawingPanel saving animated GIF: " +
    ...
    public static boolean mainIsActive() {
        ...
        for (int i = 0; i < threads.length; i++) {
            ...
            if (DEBUG) System.out.println("    partA.DrawingPanel.mainIsActive(): " + thread.getName() + ", priority=" + thread.getPriority() + ", alive=" + thread.isAlive() + ", stack=" + java.util.Arrays.toString(thread.getStackTrace()));
    ...
    private long createTime;               // time at which partA.DrawingPanel was constructed
    ...
    public DrawingPanel(int width, int height) {
        ...
        if (DEBUG) System.out.println("partA.DrawingPanel(): going to grab lock");
        synchronized (LOCK) {
            instances++;
            instanceNumber = instances;  // each partA.DrawingPanel stores its own int number
            
            if (shutdownThread == null && !usingDrJava()) {
                if (DEBUG) System.out.println("partA.DrawingPanel(): starting idle thread");
                    ...
                    public void run() {
                        ...
                        try {
                            while (true) {
                                ...
                                save |= shouldSave();
                                if (DEBUG) System.out.println("partA.DrawingPanel idle thread: instances=" + instances + ", save=" + save + ", main active=" + mainIsActive());
                                if ((instances == 0 || save) && !mainIsActive()) {
                                    try {
                                        System.exit(0);
                                    } catch (SecurityException sex) {
                                        if (DEBUG) System.out.println("partA.DrawingPanel idle thread: unable to exit program: " + sex);
                                    }
                                }
                            ...
                            if (DEBUG) System.out.println("partA.DrawingPanel idle thread: exception caught: " + e);
                        }
                    }
                });
                // shutdownThread.setPriority(Thread.MIN_PRIORITY);
                shutdownThread.setName("partA.DrawingPanel-shutdown");
        ...
        if (DEBUG) System.out.println("partA.DrawingPanel(w=" + width + ",h=" + height + ",anim=" + isAnimated() + ",graph=" + isGraphical() + ",save=" + shouldSave());
        ...
        if (isGraphical()) {
            ...
            try {
                Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
                    ...
                    public void run() {
                        if (DEBUG) System.out.println("partA.DrawingPanel.run(): Running shutdown hook");
                        if (DEBUG) System.out.println("partA.DrawingPanel shutdown hook: instances=" + instances);
                        try {
                ...
                if (DEBUG) System.out.println("partA.DrawingPanel(): unable to add shutdown hook: " + e);
            }
        }
    }
...
//    }
    ...
    /*
     * Compares the current partA.DrawingPanel image to an image file on disk.
     */
    ...
    /*
     * Compares the current partA.DrawingPanel image to an image file on the web.
     */
    ...
    /*
     * Closes the partA.DrawingPanel and exits the program.
     */
    private void exit() {
        if (isGraphical()) {
    ...
    private BufferedImage getImage() {
        ...
        // if (DEBUG) System.out.println("partA.DrawingPanel getImage setting background to " + backgroundColor);
        g.setColor(backgroundColor);
     ...
     * extracted from this by calling partA.DrawingPanel.getRed, getGreen, and getBlue.
     * If nothing has been explicitly drawn on this particular pixel, the panel's
     * background color is returned.
     * See also: getPixel.
     * @param x x-coordinate of pixel to retrieve
    ...
    /**
     * Returns the colors of all pixels in this partA.DrawingPanel as a 2-D array
     * of Color objects.
     ...
     * the partA.DrawingPanel is run 'headless' without a GUI, often for scripting
    ...
    public Image loadImage(String filename) {
    	...
    	if (!(new File(filename)).exists()) {
            throw new RuntimeException("partA.DrawingPanel.loadImage: File not found: " + filename);
        }
...
*/
    ...
    public void saveAnimated(String filename) throws IOException {
        ...
        if (DEBUG) System.out.println("partA.DrawingPanel.saveAnimated(" + filename + ")");
    ...
    private void saveAsAnimated() {
        ...
        if (filename != null) {
            try {
                ...
                JOptionPane.showMessageDialog(frame, 
                    "Due to constraints about how partA.DrawingPanel works, you'll need to\n" +
                    "re-run your program.  When you run it the next time, partA.DrawingPanel will \n" +
    ...
    /**
     * Sets the colors of all pixels in this partA.DrawingPanel to the colors
     ...
     * If the partA.DrawingPanel is not showing on the screen, has no effect.
     ...
     * lambda functions to handle mouse events that occur in a partA.DrawingPanel.
     */
    @FunctionalInterface
    ...
    /**
     * This functional interface is provided to allow Java 8 clients to write
     * lambda functions to handle key events that occur in a partA.DrawingPanel.
     */
    @FunctionalInterface
    ...
    private class DPWindowListener extends WindowAdapter {
        // called when partA.DrawingPanel closes, to potentially exit the program
    ...
    // inner class to do the actual drawing onto the partA.DrawingPanel
